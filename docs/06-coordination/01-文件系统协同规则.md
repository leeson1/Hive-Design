...
- 保证对象可定位、状态可审计、恢复可重建。
- 一致性策略与事务边界见 `02-Consistency-and-Transaction-Boundaries.md`。
...
### State Storage Layering Suggestion

- `Object Store`：承载 Directive、Plan、Task、AgentRun、Issue 等结构化对象。
- `State Store`：承载对象当前状态与索引。
- `Artifact Store`：承载补丁、报告、截图、测试结果、handoff 附件。
- `Log Store`：承载原始命令输出、终端日志、执行轨迹。

规则：

- 设计阶段仍保留文件系统语义，便于阅读、审计、版本管理。
- 实现阶段可演进为 `SQLite / Postgres + filesystem / object storage`。
- 无论底层实现如何变化，对象语义与引用关系必须保持稳定。
