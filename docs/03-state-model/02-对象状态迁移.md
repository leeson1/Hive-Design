...
- 更完整的对象状态迁移总表见 `../08-appendix/10-State-Transition-Tables.md`。

### Project Main Lifecycle
...
- Decision queue 未清空时，Phase 不得退出。

## Protocol Steps

1. 从 Idea / Brief / Plan 进入当前 Phase。
2. 将 Phase 拆分为 Task 并派发 Execution。
3. Execution 完成后进入 Validation。
4. 检查 All tasks complete、Evaluation passed、No blocking issues、Decision queue empty。
5. gate 全部通过后进入 Completion 或下一 Phase。

## Mermaid Diagram

### Hive Project State Machine
...
stateDiagram-v2
    ...
    Validation --> Completion: all tasks complete / evaluation passed / no blocking issues / decision queue empty
    Validation --> Task: fail or rework
```
